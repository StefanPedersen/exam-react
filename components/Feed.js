...
export class Feed extends Component {
    ...
    refreshFeed = res => this.setState({props:this.props})
   

   render() {
    const {elements, isLoaded } = this.state;
    
    const slug = this.state.props
   

    let filteroption = ''
    if (slug.props == 'mobilt-bredband'){
        filteroption = 'mobilt'
    }
    if (slug.props == 'fiber-internet'){
        filteroption = 'Fiber'  
    }


    function regex(name){
        let arr = name.match(/\d{1,4}.{0,2}(?:GB|TB)/) || ["Ubegrænset"] //Look for GB in Mobile name, return if present else ubegrænset
        return arr[0];
        }
    ...
    const feedOutput = elements.filter(element => element.name.includes(filteroption)).map(element =>  
